...
namespace gfx
{
	...
	public:
		...
		void add_line(v3f32 start, v3f32 end, v4f32 color, float duration = 0.f, bool foreground = true)
		{
			...
			const auto renderList = [&](VertexList& list, bool depthEnabled, GLenum type)
				{
					...
					glUniformMatrix4fv(shaderMatrixLocation, 1, GL_FALSE, glm::value_ptr(mvpMatrix));
					...
					if (list.dirty)
					{
						glBufferData(GL_ARRAY_BUFFER, list.vertices.size() * sizeof(VertexRGBA), list.vertices.data(), GL_DYNAMIC_DRAW);
						list.dirty = false;
					}
	...
	{
		gfx::DebugRenderer::get().add_line(start, end, color, duration, foreground);
	}
	...
	inline void aabb(v3f32 center, v3f32 extent, v4f32 color, float duration = 0.f, bool foreground = true)
	{
		...
		renderer.add_line(corners[0], corners[1], color, duration, foreground);
		renderer.add_line(corners[1], corners[2], color, duration, foreground);
		renderer.add_line(corners[2], corners[3], color, duration, foreground);
		renderer.add_line(corners[3], corners[0], color, duration, foreground);

		renderer.add_line(corners[4], corners[5], color, duration, foreground);
		renderer.add_line(corners[5], corners[6], color, duration, foreground);
		renderer.add_line(corners[6], corners[7], color, duration, foreground);
		renderer.add_line(corners[7], corners[4], color, duration, foreground);

		renderer.add_line(corners[0], corners[4], color, duration, foreground);
		renderer.add_line(corners[1], corners[5], color, duration, foreground);
		renderer.add_line(corners[2], corners[6], color, duration, foreground);
		renderer.add_line(corners[3], corners[7], color, duration, foreground);
	}
